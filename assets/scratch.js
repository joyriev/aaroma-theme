(function () {
  ...
  function scheduleTick() {
    ...
    const nextDelay = Math.max(
      800,
      TICK_BASE + rand(-TICK_JITTER, TICK_JITTER),
    );
  ...
  function launchConfetti() {
    ...
    function confettiPiece() {
      ...
      const fall = 1200 + Math.random() * 800;
      ...
      div.animate(
        [
          { transform: div.style.transform, top: "-10px" },
          {
            ...
            top: "110vh",
            left: `calc(${div.style.left} + ${drift}px)`,
          },
        ],
    ...
    function burst(cx, cy) {
      const root =
        document.getElementById("scratch-fireworks") || document.body;
      ...
      for (let i = 0; i < particles; i++) {
        ...
        dot.animate(
          [
            ...
            {
              transform:
                "translate(" + (x2 - cx) + "px," + (y2 - cy) + "px) scale(1)",
              opacity: 1,
              offset: 0.7,
            },
            {
              transform:
                "translate(" + (x2 - cx) + "px," + (y2 - cy) + "px) scale(0.2)",
              opacity: 0,
              offset: 1,
            },
          ],
  ...
  if (cta) {
    cta.addEventListener("click", function (e) {
      e.preventDefault();
      const cfgLink = "https://aaroma.co/pages/build-your-bundle-prize";
      const openNew = section.getAttribute("data-cta-new-tab") === "true";
      try {
        if (openNew) window.open(cfgLink, "_blank");
        else window.location.assign(cfgLink);
      } catch (_e) {
        window.location.href = cfgLink;
      }
    });
  }
})();
